...
LearningBot::LearningBot(Raven_Game* world, Vector2D pos) : Raven_Bot(world, pos, script->GetDouble("Rambo_Scale"), 9) {
...
void LearningBot::Update()
{
	...
	if (!isPossessed())
	{
		...
		if (m_pTargSys->isTargetPresent()) {
			...
			m_vecObservation.push_back((Pos().Distance(m_pTargSys->GetTarget()->Pos())));										// Distance entre le bot et son target
			m_vecObservation.push_back(m_pTargSys->isTargetWithinFOV());														// Si le target est dans le FOV
			m_vecObservation.push_back(m_pWeaponSys->GetAmmoRemainingForWeapon(m_pWeaponSys->GetCurrentWeapon()->GetType()));	// Balle restante
			m_vecObservation.push_back(m_pWeaponSys->GetCurrentWeapon()->GetType());											// Arme utilisé
			m_vecObservation.push_back((Health()));																				// Vie du bot


			std::vector<double> output = m_ModeleAppris.Update(m_vecObservation);												// Nous retourne si on tire ou non
